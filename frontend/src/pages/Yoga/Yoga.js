...
function Yoga() {
    ...
    if (
      ...
      try {
        ...
        classification.array().then((data) => {
          ...
          const isPoseCorrect = data[0][classNo] > 0.97;

          if (isPoseCorrect) {
            skeletonColor = "rgb(0,255,0)"; // Green skeleton for correct pose
            ...
            skeletonColor = "rgb(255,255,255)"; // White skeleton for incorrect pose
            if (!countAudio.paused) {
              // Only pause if audio is playing
              countAudio.pause();
              countAudio.currentTime = 0; // Reset to start for next play
            }
          }
        });
      } catch (err) {
  ...
  if (isStartPose) {
    return (
      <div className="yoga-container" style={{ paddingTop: "20px" }}>
